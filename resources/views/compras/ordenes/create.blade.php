...
@section('content')
    <div class="max-w-6xl mx-auto" x-data="ordenCompra()">
        ...
        <div
            ...
            <div>
                <h2 class="text-2xl font-bold text-slate-800">Nueva Orden de Compra</h2>
                <span class="text-sm text-slate-500">Generación de Pedido a Proveedor</span>
            </div>
            <div class="text-right">
                <div class="text-xs font-bold text-slate-400 uppercase">Fecha Actual</div>
                <div class="font-mono text-lg text-slate-700">{{ date('d/m/Y') }}</div>
            </div>
        </div>
    ...
    <script>
        function ordenCompra() {
            return {
                ...
                lugar_entrega: '',
                observaciones: '',
                rows: [{ item_id: null, descripcion: '', cantidad: 1, costo: 0 }],
                ...
                async save() {
                    ...
                    if (!this.proveedor_id) {
                        return Swal.fire({
                            ...
                            text: 'Por favor seleccione un proveedor de la lista.',
                    ...
                    try {
                        let response = await fetch('{{ route('ordenes_compra.store') }}', {
                            method: 'POST',
                            headers: {
                        ...
                        if (result.success) {
                            ...
                            Swal.fire({
                                icon: 'success',
                                title: '¡Orden Creada!',
                                text: 'La orden de compra se ha registrado exitosamente.',
                            ...
                            // 5. ERROR DE LÓGICA (MODAL ROJO)
                            Swal.fire({
                                icon: 'error',
                                title: 'Error al Guardar',
                                text: result.msg || 'Hubo un problema desconocido.',
                                confirmButtonColor: '#ef4444'
                            });
                        }
                    } catch (error) {
                        console.error(error);
                        // 6. ERROR DE RED (MODAL ROJO)
                        Swal.fire({
                            icon: 'error',
                            title: 'Error de Sistema',
                            text: 'No se pudo conectar con el servidor. Revise la consola.',
                            footer: '<span class="text-slate-400 text-xs">Error 500 / Network Error</span>'
                        });
                    }
                }
            }
